...
import {
  ...
  ScrollView, ActivityIndicator, TextInput, Image, Alert
} from 'react-native';
...
export default function GestionScreen() {
  ...
  const cargarAnimes = async () => {
    ...
    try {
      ...
      Alert.alert('Error', 'No se pudieron cargar los animes');
    } finally {
  ...
  const handleEliminarAnime = (anime) => {
    Alert.alert(
      '🗑️ Eliminar Anime',
      `¿Eliminar "${anime.nombre}" y todos sus personajes?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Error', e.response?.data?.error || 'Error al eliminar');
            }
          }
        }
      ]
  ...
  const handleEliminarPersonaje = (personaje, animeId) => {
    Alert.alert(
      '🗑️ Eliminar Personaje',
      `¿Eliminar a "${personaje.nombre}"?`,
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          text: 'Eliminar', style: 'destructive',
  ...
  const handleCrearPersonaje = async (animeId) => {
    ...
    if (imagenesUrls.length < 4) {
      Alert.alert('Error', 'Sube las 4 imágenes del personaje');
      return;
    }
    ...
    try {
      ...
      resetFormPj();
      Alert.alert('✅ Éxito', `Personaje "${nombrePj}" creado`);
    } catch (e) {
      Alert.alert('Error', e.response?.data?.error || 'Error al crear');
    } finally {
        ...
        {loadingAnimes ? (
          ...
          animes.map((anime, index) => (
            <View key={anime.id} style={styles.animeCard}>
              <TouchableOpacity style={styles.animeHeader} onPress={() => toggleAnime(anime)} activeOpacity={0.8}>
                ...
                <View style={styles.animeActions}>
                  <TouchableOpacity style={styles.deleteBtn} onPress={() => handleEliminarAnime(anime)}>
                    ...
                    <>
                      ...
                      {personajes[anime.id]?.map(pj => (
                        <View key={pj.id} style={styles.personajeRow}>
                          ...
                          <TouchableOpacity style={styles.deletePersonajeBtn} onPress={() => handleEliminarPersonaje(pj, anime.id)}>
                            <Text style={styles.deleteBtnText}>🗑️</Text>
                          </TouchableOpacity>
                        </View>
                      ))}
                      ...
                      {agregarPjAnimeId !== anime.id ? (
                        <TouchableOpacity style={styles.agregarPjBtn} onPress={() => setAgregarPjAnimeId(anime.id)}>
                        ...
                        <View style={styles.formPj}>
                          ...
                          <TextInput style={styles.formInput} placeholder="Nombre *" placeholderTextColor="#555" value={nombrePj} onChangeText={setNombrePj} />
                          <TextInput style={styles.formInput} placeholder="Edad" placeholderTextColor="#555" value={edadPj} onChangeText={setEdadPj} keyboardType="numeric" />
                          <TextInput style={styles.formInput} placeholder="Poder especial" placeholderTextColor="#555" value={poderPj} onChangeText={setPoderPj} />

                          <Text style={styles.formLabel}>🖼️ Imágenes (4 requeridas)</Text>
                          <View style={styles.imageGrid}>
                            {imagenesPj.map((img, idx) => (
                              <TouchableOpacity key={idx} style={styles.imageSlot} onPress={() => seleccionarImagen(idx)} disabled={uploadingIndex === idx}>
                                {uploadingIndex === idx ? (
                          ...
                          <TouchableOpacity onPress={() => handleCrearPersonaje(anime.id)} disabled={loadingCrearPj} activeOpacity={0.85}>
                            <LinearGradient colors={['#7c3aed', '#4f46e5']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.formBtn}>
                              {loadingCrearPj ? <ActivityIndicator color="#fff" /> : <Text style={styles.formBtnText}>🚀 Crear Personaje</Text>}
                            </LinearGradient>
                          </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  circle1: { position: 'absolute', width: 300, height: 300, borderRadius: 150, backgroundColor: '#7c3aed15', top: -80, right: -80 },
  circle2: { position: 'absolute', width: 200, height: 200, borderRadius: 100, backgroundColor: '#c026d315', bottom: 100, left: -60 },
  ...
  animeCard: { backgroundColor: '#ffffff08', borderRadius: 16, borderWidth: 1, borderColor: '#ffffff10', marginBottom: 12, overflow: 'hidden' },
  ...
  personajesContainer: { borderTopWidth: 1, borderTopColor: '#ffffff10', paddingHorizontal: 16 },
  emptyText: { color: '#555', fontSize: 13, padding: 12, textAlign: 'center' },
  personajeRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#ffffff08' },
  ...
  agregarPjBtn: { margin: 12, padding: 12, borderRadius: 10, borderWidth: 1, borderColor: '#7c3aed', alignItems: 'center', borderStyle: 'dashed' },
  agregarPjBtnText: { color: '#7c3aed', fontWeight: '600', fontSize: 13 },
  formPj: { padding: 12 },
  formTitle: { color: '#fff', fontWeight: '800', fontSize: 16, marginBottom: 12 },
  formInput: { backgroundColor: '#00000050', borderRadius: 10, borderWidth: 1, borderColor: '#ffffff10', color: '#fff', paddingHorizontal: 14, paddingVertical: 10, marginBottom: 10, fontSize: 14 },
  ...
  imageSlot: { width: '47%', height: 120, borderRadius: 10, backgroundColor: '#ffffff08', borderWidth: 1, borderColor: '#ffffff15', borderStyle: 'dashed', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' },
